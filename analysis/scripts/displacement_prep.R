...
# Find bounding box of surveyed sites adding 1km
outcoords <- st_bbox(surveyed) + 1000
...
plot(st_centroid(surveyed), pch = 19, cex = 0.5, colour = "black", add = TRUE)
plot(isobases, add = TRUE)
plot(st_geometry(bmap), add = TRUE)
...
ggplot() +
  ...
  geom_sf(data = st_centroid(surveyed), pch = 21, fill = "red") +
  theme_classic()
